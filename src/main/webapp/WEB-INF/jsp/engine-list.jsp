...
<div class="row">
    ...
    <div class="container">
        <h3 class="text-center">List of Car Engines</h3>
        <hr>
        <div class="container text-left">

            <a href="<%=request.getContextPath()%>/engine/new" class="btn btn-success">Add
                New Car Engine</a>
        </div>
        <br>
        <table class="table table-bordered">
            ...
            <tbody>
            <!--   for (Todo todo: todos) {  -->
            <jsp:useBean id="listEngines" scope="request" type="java.util.List"/>
            <c:forEach var="engine" items="${listEngines}">
                <tr>
                    ...
                    <td>
                        <c:out value="${engine.getManufacturerId()}" />
                    </td>
                    <td>
                        <c:out value="${engine.getName()}" />
                    </td>
                    <td><a href="<%=request.getContextPath()%>/engine/edit?id=<c:out value='${engine.getId()}' />">Edit</a> &nbsp;&nbsp;&nbsp;&nbsp; <a href="<%=request.getContextPath()%>/engine/delete?id=<c:out value='${engine.getId()}' />">Delete</a></td>
                </tr>
            </c:forEach>
            <!-- } -->
            </tbody>
